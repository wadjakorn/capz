...
- **Settings schema:** **any** change to persisted settings — a new key, a new enum value, a rename — must bump `CONFIG_SCHEMA_VERSION` (an identity step is fine for additions); downgrade protection only works across a version bump, and `src/lib/config.shape.test.ts` fails when keys or enum values change without one (regenerate `shape.v<N>.json` on the bump). Bumping `CONFIG_SCHEMA_VERSION` (`src/lib/config.ts`) requires a `CONFIG_MIGRATIONS` step (import-time check fails tests otherwise) that carries user values across renames, a fixture in `src/lib/__fixtures__/config/`, and a check of the Rust readers listed above `CONFIG_MIGRATIONS` — Rust reads `config.json` before the webview migrates it. A store written by a newer capz is never rewritten in full (`persist()` in `src/stores/settings.ts`).
