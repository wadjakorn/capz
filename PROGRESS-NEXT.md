...
- [ ] **macOS update invalidates Screen Recording grant** — after installing a new version, capture fails as if permission never granted, yet System Settings → Privacy & Security → Screen Recording still lists `capz` as allowed. TCC likely keys the grant on code-signature / bundle identity that changes across updates (or stale entry survives but new binary not trusted). Repro on each `.app` update. Workaround: toggle capz off + on in System Settings, or remove + re-add. Investigate signing identity stability across releases; consider prompting user to re-toggle on first launch after update if `has_screen_recording_permission` reports false despite prior grant.
