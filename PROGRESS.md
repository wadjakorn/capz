...
2. ~~**Editor show/open shortcut.** Global hotkey to focus/show editor without going through tray. Likely default `CmdOrCtrl+Alt+Shift+0` or similar; user-configurable in Settings → Shortcuts. Wires into existing `windows::show_editor` path.~~ Done 2026-05-23 — `hotkeys.showEditor` config (default `CmdOrCtrl+Alt+Shift+0`), registered in `shortcuts.rs` alongside capture shortcuts, fires `windows::show_editor` directly. Settings → Shortcuts row added.
...
3. ~~**Iconified buttons.** Replace bare-text buttons (toolbar tool buttons, Save/Copy/Cancel, Settings tab triggers, onboarding actions) with `lucide-react` icons + tooltip labels. Keep text on primary CTAs.~~ Partially done 2026-05-23 — editor toolbar tools (Select/Arrow/Rect/Text/Blur/Sticker/Pin) + Undo/Redo now icon-only with title+aria-label; Copy/Save/Save&Copy keep text + icon. Settings tab triggers + onboarding actions still bare text (deferred).
