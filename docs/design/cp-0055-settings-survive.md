...
**Limits found in review (PR #88):**
- **Downgrade protection depends on `schemaVersion`.** An older build treats a store with the *same* version as its own. So a persisted key or enum value added without a version bump is still stripped or reset on that build's next write. This has happened before: `backdrop`, `commandRing` and `defaultShape` arrived between v0.5.1 and v0.9.2 while the version stayed at 1. The rule is now that **any** persisted change bumps `CONFIG_SCHEMA_VERSION` (an identity step is fine). It's written in CLAUDE.md and in the `CONFIG_MIGRATIONS` header. `config.shape.test.ts` enforces this: it pins the keys of `DEFAULT_CONFIG`, plus every validated leaf (including `lastUsed.*`) and its enum values from `persistedShape()`, to `shape.v<N>.json`. A new key or enum value without a bump fails `pnpm test:unit`. Not covered: a change to what a non-enum validator accepts (for example, widening a number range).
- **Self-heal backups on the same version.** The file is backed up to `lastRewrite`, which each rewrite overwrites. Only upgrades keep a permanent per-version snapshot (`app@v<N>`).
